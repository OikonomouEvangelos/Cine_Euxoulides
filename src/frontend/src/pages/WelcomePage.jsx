...
const WelcomePage = () => {
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      // Αποθηκεύουμε το token & τα στοιχεία χρήστη (ΣΥΜΠΕΡΙΛΑΜΒΑΝΟΜΕΝΟΥ ΤΟΥ ID)
      if (data.token) {
        localStorage.setItem('token', data.token);
        // ΠΡΟΣΘΗΚΗ: Αποθηκεύουμε το ID για να δουλέψουν οι Φίλοι
        // Ελέγχουμε αν το backend το στέλνει ως 'userId' ή 'id'
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    try {
      ...
      // Αν ο backend μας έστειλε JWT token, το αποθηκεύουμε
      if (data.token) {
        localStorage.setItem('token', data.token);
        // ΠΡΟΣΘΗΚΗ: Αποθηκεύουμε και εδώ το ID
        localStorage.setItem('userId', data.userId || data.id);
        localStorage.setItem('userEmail', data.email);
        localStorage.setItem('userFirstName', data.firstName);
