...
const MovieWheel = ({ genreId, title = "Δεν ξέρεις τι να δεις;" }) => {
  ...
  return (
    <>
      <button className="wheel-open-btn" onClick={() => { setIsOpen(true); loadCandidates(); }}>
        🎡 Τροχός
      </button>
        ...
        <div className="wheel-modal-overlay" onClick={() => setIsOpen(false)}>
          <div className="wheel-modal" onClick={(e) => e.stopPropagation()}>
            <div className="wheel-header">
              <h3>{title}</h3>
              <button className="wheel-close" onClick={() => setIsOpen(false)}>✕</button>
            </div>
            ...
            <div className="wheel-actions">
              ...
              <button className="wheel-btn secondary" onClick={loadCandidates} disabled={isSpinning}>
                Ανανέωση Ταινιών
              </button>
            </div>
          </div>
